...
public class JobApplicationDaoImpl extends BaseDaoImpl<JobApplicationRecord, JobApplication> implements JobApplicationDao {
	...
	public ProfileBasicRecord getBasicByUserId(int userId) throws Exception {
		...
		try {
			conn = DBConnHelper.DBConn.getConn();
			DSLContext create = DBConnHelper.DBConn.getJooqDSL(conn);
			Result<Record> result = create.select(ProfileBasic.PROFILE_BASIC.fields())
					.from(ProfileBasic.PROFILE_BASIC).join(ProfileProfile.PROFILE_PROFILE)
	...
	public int getViewNumber(long positionId, long userId) throws Exception {
		...
		try {
			...
			Result<Record1<Integer>> result = create.select(CandidatePosition.CANDIDATE_POSITION.VIEW_NUMBER)
					...
					.on(CandidatePosition.CANDIDATE_POSITION.CANDIDATE_COMPANY_ID.equal(CandidateCompany.CANDIDATE_COMPANY.ID))
					.where(CandidatePosition.CANDIDATE_POSITION.POSITION_ID.equal((int)positionId)).and(CandidateCompany.CANDIDATE_COMPANY.SYS_USER_ID.equal((int)userId)).limit(0).fetch();
			if(result != null && result.size() > 0) {
