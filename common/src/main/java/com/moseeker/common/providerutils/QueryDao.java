...
public class QueryDao {
    ...
    public <S, R> S getData(CrudQuery<S, R> dao){
        return dao.getData(commonQuery);
    }

    public <S, R, T> T getData(CrudQuery<S, R> dao, Class<T> tClass){
        return dao.getData(commonQuery, tClass);
    }

    public <S, R, T> List<S> getDatas(CrudQuery<S, R> dao){
        return dao.getDatas(commonQuery);
    }

    public <S, R, T> List<T> getDatas(CrudQuery<S, R> dao, Class<T> tClass){
