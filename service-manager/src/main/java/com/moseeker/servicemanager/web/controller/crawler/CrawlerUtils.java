...
public class CrawlerUtils {
	...
	public Response fetchFirstResume(String userName, String password, String token, int type, int lang, int source,
		...
		boolean allowImport = isAllowImport(user_id, channelType);
		if (allowImport) {
			String result = null;
	 ...
	 * @return
	 */
	private boolean isAllowImport(int user_id, ChannelType channelType) {
		...
		try {
			return redisClient.isAllowed(PROFILE_IMPORT_UPPER_LIMIT, userIdStr,
					Constant.PROFILE_IMPORT_UPPER_LIMIT, "1");
