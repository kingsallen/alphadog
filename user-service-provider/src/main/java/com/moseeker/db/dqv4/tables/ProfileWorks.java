/**
 * This class is generated by jOOQ
 */
package com.moseeker.db.dqv4.tables;


import com.moseeker.db.dqv4.Dqv4;
import com.moseeker.db.dqv4.Keys;
import com.moseeker.db.dqv4.tables.records.ProfileWorksRecord;

import java.sql.Timestamp;
import java.util.Arrays;
import java.util.List;

import javax.annotation.Generated;

import org.jooq.Field;
import org.jooq.Identity;
import org.jooq.Table;
import org.jooq.TableField;
import org.jooq.UniqueKey;
import org.jooq.impl.TableImpl;
import org.jooq.types.UInteger;


/**
 * Profile的个人作品
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.7.3"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class ProfileWorks extends TableImpl<ProfileWorksRecord> {

	private static final long serialVersionUID = 1494179011;

	/**
	 * The reference instance of <code>dqv4.profile_works</code>
	 */
	public static final ProfileWorks PROFILE_WORKS = new ProfileWorks();

	/**
	 * The class holding records for this type
	 */
	@Override
	public Class<ProfileWorksRecord> getRecordType() {
		return ProfileWorksRecord.class;
	}

	/**
	 * The column <code>dqv4.profile_works.id</code>. 主key
	 */
	public final TableField<ProfileWorksRecord, UInteger> ID = createField("id", org.jooq.impl.SQLDataType.INTEGERUNSIGNED.nullable(false), this, "主key");

	/**
	 * The column <code>dqv4.profile_works.profile_id</code>. profile.id
	 */
	public final TableField<ProfileWorksRecord, UInteger> PROFILE_ID = createField("profile_id", org.jooq.impl.SQLDataType.INTEGERUNSIGNED.nullable(false).defaulted(true), this, "profile.id");

	/**
	 * The column <code>dqv4.profile_works.name</code>. 作品名称
	 */
	public final TableField<ProfileWorksRecord, String> NAME = createField("name", org.jooq.impl.SQLDataType.VARCHAR.length(256).nullable(false).defaulted(true), this, "作品名称");

	/**
	 * The column <code>dqv4.profile_works.url</code>. 作品网址
	 */
	public final TableField<ProfileWorksRecord, String> URL = createField("url", org.jooq.impl.SQLDataType.VARCHAR.length(1000).nullable(false).defaulted(true), this, "作品网址");

	/**
	 * The column <code>dqv4.profile_works.description</code>. 作品描述
	 */
	public final TableField<ProfileWorksRecord, String> DESCRIPTION = createField("description", org.jooq.impl.SQLDataType.VARCHAR.length(1000).nullable(false).defaulted(true), this, "作品描述");

	/**
	 * The column <code>dqv4.profile_works.create_time</code>. 创建时间
	 */
	public final TableField<ProfileWorksRecord, Timestamp> CREATE_TIME = createField("create_time", org.jooq.impl.SQLDataType.TIMESTAMP.nullable(false).defaulted(true), this, "创建时间");

	/**
	 * The column <code>dqv4.profile_works.update_time</code>. 更新时间
	 */
	public final TableField<ProfileWorksRecord, Timestamp> UPDATE_TIME = createField("update_time", org.jooq.impl.SQLDataType.TIMESTAMP.nullable(false).defaulted(true), this, "更新时间");

	/**
	 * Create a <code>dqv4.profile_works</code> table reference
	 */
	public ProfileWorks() {
		this("profile_works", null);
	}

	/**
	 * Create an aliased <code>dqv4.profile_works</code> table reference
	 */
	public ProfileWorks(String alias) {
		this(alias, PROFILE_WORKS);
	}

	private ProfileWorks(String alias, Table<ProfileWorksRecord> aliased) {
		this(alias, aliased, null);
	}

	private ProfileWorks(String alias, Table<ProfileWorksRecord> aliased, Field<?>[] parameters) {
		super(alias, Dqv4.DQV4, aliased, parameters, "Profile的个人作品");
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public Identity<ProfileWorksRecord, UInteger> getIdentity() {
		return Keys.IDENTITY_PROFILE_WORKS;
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public UniqueKey<ProfileWorksRecord> getPrimaryKey() {
		return Keys.KEY_PROFILE_WORKS_PRIMARY;
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public List<UniqueKey<ProfileWorksRecord>> getKeys() {
		return Arrays.<UniqueKey<ProfileWorksRecord>>asList(Keys.KEY_PROFILE_WORKS_PRIMARY);
	}

	/**
	 * {@inheritDoc}
	 */
	@Override
	public ProfileWorks as(String alias) {
		return new ProfileWorks(alias, this);
	}

	/**
	 * Rename this table
	 */
	public ProfileWorks rename(String name) {
		return new ProfileWorks(name, null);
	}
}
