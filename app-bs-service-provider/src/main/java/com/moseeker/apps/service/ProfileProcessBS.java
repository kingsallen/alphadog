...
public class ProfileProcessBS {
	...
	public void sendTemplate(int userId, String userName, int companyId, int status, String positionName, int applicationId, TemplateMs tm) {
		...
		if (msInfo != null) {
			...
			templateNoticeStruct.setUrl(MessageFormat.format(tm.getUrl(), ConfigPropertiesUtil.getInstance().get("platform.url", String.class), signature, String.valueOf(applicationId)));
	        try {
