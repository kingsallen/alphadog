		...
		try {
			if(intentionIds != null && intentionIds.size() > 0) {
				...
				for(int i=0; i<intentionIds.size(); i++) {
					if(i == 0) {
						selectCondition = select.where(ProfileIntentionCity.PROFILE_INTENTION_CITY.PROFILE_INTENTION_ID.equal(UInteger.valueOf(intentionIds.get(i))));
					} else {
						selectCondition.or(ProfileIntentionCity.PROFILE_INTENTION_CITY.PROFILE_INTENTION_ID.equal(UInteger.valueOf(intentionIds.get(i))));
					}
				}
