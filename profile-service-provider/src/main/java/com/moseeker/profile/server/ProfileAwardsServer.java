 ...
 * @version Beta
 */
public class ProfileAwardsServer {
	
	private static Logger LOGGER = LoggerFactory.getLogger(ProfileAwardsServer.class);
	
	public static void main(String[] args) {

		try {
			AnnotationConfigApplicationContext acac = initSpring();
			Server server = new Server(ProfileAwardsServer.class,
					ServerNodeUtils.getPort(args),
					acac.getBean(ProfileAwardsServicesImpl.class));
			server.start(); // 启动服务，非阻塞

			synchronized (ProfileAwardsServer.class) {
				while (true) {
					try {
