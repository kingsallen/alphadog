 ...
 * @version Beta
 */
public class ProfileWorkExpServer {
	
	private static Logger LOGGER = LoggerFactory.getLogger(ProfileWorkExpServer.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			synchronized (ProfileWorkExpServer.class) {
				while (true) {
					try {
						ProfileWorkExpServer.class.wait();
                    } catch (Exception e) {
                        LOGGER.error(" service provider ProfileWorkExpServer error", e);
                    }
				}
			}
		} catch (Exception e) {
