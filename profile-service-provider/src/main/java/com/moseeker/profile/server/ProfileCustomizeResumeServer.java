 ...
 * @version Beta
 */
public class ProfileCustomizeResumeServer {
	
	private static Logger LOGGER = LoggerFactory.getLogger(ProfileCustomizeResumeServer.class);
	
	public static void main(String[] args) {

		try {
			AnnotationConfigApplicationContext acac = initSpring();
			Server server = new Server(ProfileCustomizeResumeServer.class,
					ServerNodeUtils.getPort(args),
					acac.getBean(ProfileCustomizeResumeServicesImpl.class));
			server.start(); // 启动服务，非阻塞

			synchronized (ProfileCustomizeResumeServer.class) {
				while (true) {
					try {
