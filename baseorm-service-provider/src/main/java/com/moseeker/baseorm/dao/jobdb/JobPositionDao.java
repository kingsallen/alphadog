...
public class JobPositionDao extends JooqCrudImpl<JobPositionDO, JobPositionRecord> {
    ...
    public List<PositionDetails> hotPositionDetailsList(CommonQuery commonQuery) {
        ...
        if (hashMap.get("company_id") != null) {
            ...
            record.where(jp.COMPANY_ID.equal(Integer.valueOf(commonQuery.getEqualFilter().get("company_id")))).and(jp.STATUS.equal((byte) 0)).orderBy(fields);
        }
        // 分页
        int page = 1;
        int per_page = 20;
