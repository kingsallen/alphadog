...
public class ProfileProfileDaoThriftService implements ProfileProfileDao.Iface {
    ...
    @Override
    public Response getResourceByApplication(String downloadApi, String password, int companyId, int sourceId,
                                             int atsStatus, boolean recommender, boolean dl_url_required) throws TException {
        try {
            return profileDao.getResourceByApplication(downloadApi, password, companyId, sourceId, atsStatus,
                    recommender, dl_url_required);
        } catch (Exception e) {
